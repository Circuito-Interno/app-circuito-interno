...
/* =========================================================
   STREAMS ORIGINAIS (Circuito Interno Direto sem Proxy)
   ========================================================= */
...
export default function App() {
  ...
  /* =========================================================
     MEDIA SESSION (Controlos no ecrã de bloqueio do iOS)
     ========================================================= */
  ...
  /* =========================================================
     METADADOS DO AZURACAST (Rhoster)
     ========================================================= */
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data?.now_playing?.song) {
          ...
          updateMediaSession(songText);
        }
      }
    } catch {
      /* Falha silenciosa sem afetar áudio */
    }
  };
  ...
  const toggle = async (selectedSource?: AudioSource) => {
    ...
    try {
      ...
      setPlaying(false);
      setLoading(false);
      setError('Não foi possível iniciar a emissão.');
    }
  };
  ...
  const playerTitle = audioSource === 'marcoense' ? 'Rádio Marcoense (Em Direto)' : currentSong;
  const playerSubtitle = audioSource === 'marcoense' ? 'Sinal 93.3 FM' : 'Circuito Interno – A Sua Emissora';
      ...
      <header className="border-b border-zinc-800/50 bg-zinc-900/40 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCarMode(!carMode)} className={`px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 transition-all ${carMode ? 'bg-amber-500 text-zinc-950' : 'bg-zinc-800/80 text-zinc-300'}`}>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6 space-y-6">
        ...
        <div className={`relative overflow-hidden rounded-3xl border border-zinc-800 bg-gradient-to-b from-zinc-900/90 to-zinc-950 p-6 sm:p-8 shadow-2xl ${carMode ? 'py-12' : ''}`}>
          <div className="flex justify-center gap-2 mb-8 flex-wrap">
            <button onClick={() => toggle('circuito')} disabled={loading} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${audioSource === 'circuito' ? 'bg-orange-600 text-white' : 'bg-zinc-800/50 text-zinc-400'}`}>
              RÁDIO 24/7
            </button>

            <button onClick={() => toggle('marcoense')} disabled={loading} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${audioSource === 'marcoense' ? 'bg-red-600 text-white' : 'bg-zinc-800/50 text-zinc-400'}`}>
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <button onClick={() => toggle()} disabled={loading} aria-label={playing ? 'Pausar rádio' : 'Reproduzir rádio'} className={`rounded-full bg-gradient-to-tr from-orange-600 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white flex items-center justify-center transition-all shadow-xl ${carMode ? 'w-28 h-28' : 'w-20 h-20'}`}>
              {loading ? (
